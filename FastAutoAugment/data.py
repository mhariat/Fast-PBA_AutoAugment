...
from FastAutoAugment.archive import arsaug_policy, autoaug_policy, autoaug_paper_cifar10, random_search2048, \
    fa_reduced_imagenet, fa_reduced_cifar10, fa_wresnet28x10_cifar100, fa_wresnet40x2_cifar100,\
    fa_shake26_2x96d_cifar100, fa_wresnet40x2_cifar100_r5
...
from FastAutoAugment.pba.pba_parser import *
...
def load_dataset(data_dir, resize, dataset_name, img_type):
    ...
    if isinstance(C.get()['aug'], list):
        ...
        if C.get()['aug'] == 'random2048':
            train_transform.transforms.insert(0, Augmentation(random_search2048()))
            ...
            train_transform.transforms.insert(0, Augmentation(fa_reduced_imagenet()))
            ...
            train_transform.transforms.insert(0, Augmentation(autoaug_policy()))
    ...
    val_dataset = CustomDataset(hdf5_file=hdf5_train_path, transform=test_transform)
    test_dataset = CustomDataset(hdf5_file=hdf5_test_path, transform=test_transform)

    train_dataset.train_labels = train_dataset.labels_id
    return [train_dataset, val_dataset, test_dataset]


# def get_dataloaders(dataroot, batch, resize=32, img_type='png', horovod=False):
#     dataset_name = dataroot.split('/')[-1]
#     train_data_set, val_data_set, test_data_set = load_dataset(dataroot, resize, dataset_name, img_type)
#     if horovod:
#         import horovod.torch as hvd
#         train_sampler = DistributedStratifiedSampler(train_data_set.train_labels, num_replicas=hvd.size(), rank=hvd.rank())
#     else:
#         train_sampler = StratifiedSampler(train_data_set.train_labels)
#     train_sampler = StratifiedSampler(train_data_set.train_labels)
#     val_sampler = SubsetSampler([])
#     train_dataloader = torch.utils.data.DataLoader(train_data_set, batch_size=batch, shuffle=False,
#                                                    num_workers=32)
#     val_dataloader = torch.utils.data.DataLoader(val_data_set, batch_size=batch, shuffle=False,
#                                                  num_workers=16)
#     test_dataloader = torch.utils.data.DataLoader(test_data_set, batch_size=batch, num_workers=32, shuffle=False)
#
...
def get_dataloaders(dataset, batch, dataroot, split=0.0, split_idx=0, horovod=False):
    if 'cifar100' in dataset:
        transform_train = transforms.Compose([
            ...
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize(cifar_100['mean'], cifar_100['std']),
        ])
    ...
    if isinstance(C.get()['aug'], list):
        ...
        if C.get()['aug'] == 'random2048':
        ...
        elif C.get()['aug'] == 'fa_wresnet40x2_cifar100':
            transform_train.transforms.insert(0, Augmentation(fa_wresnet40x2_cifar100()))
        elif C.get()['aug'] == 'fa_wresnet28x10_cifar100':
            transform_train.transforms.insert(0, Augmentation(fa_wresnet28x10_cifar100()))
        elif C.get()['aug'] == 'fa_wresnet40x2_cifar100_r5':
            transform_train.transforms.insert(0, Augmentation(fa_wresnet40x2_cifar100_r5()))
        elif C.get()['aug'] == 'fa_shake26_2x96d_cifar100':
            transform_train.transforms.insert(0, Augmentation(fa_shake26_2x96d_cifar100()))
        elif C.get()['aug'] == 'arsaug':
            ...
            file_path = C.get()['hp_path']
            hp_epochs = C.get()['hp_epochs']
            epochs = C.get()['epoch']
            policies = get_policy(file_path, hp_epochs, epochs)
            transform_train.transforms.insert(0, AugmentationPba(policies))
        elif C.get()['aug'] in ['default', 'inception', 'inception320']:
            pass
        else:
            raise ValueError('not found augmentations. %s' % C.get()['aug'])
    ...
    if C.get()['cutout'] > 0:
        if C.get()['aug'] == 'pba':
            transform_train.transforms.append(CutoutNumpy(C.get()['cutout']))
        else:
    ...
    if dataset == 'cifar10':
        ...
        total_trainset.train_labels = total_trainset.targets
        ...
        # total_trainset.train_labels = total_trainset.targets
        total_trainset = torchvision.datasets.ImageFolder('{}/training_set'.format(dataroot), transform=transform_train)
        testset = torchvision.datasets.ImageFolder('{}/test_set'.format(dataroot), transform=transform_test)
        total_trainset.train_labels = total_trainset.targets
...
class CutoutNumpy(object):
    def __init__(self, length):
        self.length = length

    def __call__(self, img):
        size = self.length
        num_channels, img_height, img_width = (img.shape[0], img.shape[1], img.shape[2])
...
class AugmentationPba(object):
    ...
    def __call__(self, img):
        ...
        if count != 0:
            policy = copy.copy(self.policies[self.epoch])
            ...
            for xform in policy:
                ...
                if random.random() > probability:
                    continue
                img = apply_augment_pba(img, name, level)
                count -= 1
                if count == 0:
                    break
            return img
