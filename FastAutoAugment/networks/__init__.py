...
from FastAutoAugment.networks.pyramidskipnet import PyramidSkipNet
...
def get_model(conf, num_class=10, data_parallel=True):
    ...
    if name == 'wresnet40_2':
        ...
        model = PyramidSkipNet(depth=conf['depth'], alpha=conf['alpha'], num_classes=num_class, bottleneck=conf['bottleneck'])
